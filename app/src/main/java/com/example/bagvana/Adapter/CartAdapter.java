...
public class CartAdapter extends RecyclerView.Adapter<CartAdapter.CartViewHolder> {
    ...
    public void onBindViewHolder(@NonNull CartViewHolder holder, int position) {
        ...
        for(int i = 0; i < Utils._productListCart.size(); i++) {
            if(Utils._productListCart.get(i).getProductID() == product.getProductID()) {
                        ...
                        for(Product pd: products) {
                            if(pd.getProductID().equals(product.getProductID())) {
                                if(pd.getAmount() <= amountChanged) {
                                    ...
                                    for(int i = 0; i < Utils._productListCart.size(); i++) {
                                        if(Utils._productListCart.get(i).getProductID() == product.getProductID()) {
                                            Utils._productListCart.get(i).setAmount(amountChanged);
                                        }
                                        EventBus.getDefault().postSticky(new BillCostEvent());
                                    }
                                }
                    ...
                    new AlertDialog.Builder(context)
                            ...
                            .setPositiveButton("Đồng ý", new DialogInterface.OnClickListener() {
                                public void onClick(DialogInterface dialog, int which) {
                                    ...
                                    for(int i = 0; i < Utils._productListCart.size(); i++) {
                                        if(Utils._productListCart.get(i).getProductID() == product.getProductID()) {
                                            Utils._productListCart.remove(i);
                                        }
                                        EventBus.getDefault().postSticky(new BillCostEvent());
                                    }
                                }
        ...
        holder.checkBox_product.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                if(b) {
                    Utils._productListCart.add(product);
                    EventBus.getDefault().postSticky(new BillCostEvent());
                } else {
                    for(int i = 0; i < Utils._productListCart.size(); i++) {
                        if(Utils._productListCart.get(i).getProductID() == product.getProductID()) {
